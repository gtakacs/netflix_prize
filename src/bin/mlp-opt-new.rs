...
use netflix_prize::blend::{
    build_xy, cvk_blend, expand_specs, load_quiz_mask, save_preds, NOCLIP_OP,
};
...
fn blend_config(name: &str) -> BlendParams {
    match name {
        ...
        "mlpr1" => BlendParams { hidden: vec![64, 64], alpha: 0.05, lr: 0.0004, iters: 64, batch: 200, momentum: 0.9, tol: 1e-4, n_iter_no_change: 10, folds: 2 },
        // create_nn2: (64, 48, 32), otherwise identical.
        "mlpr2" => BlendParams { hidden: vec![64, 48, 32], alpha: 0.05, lr: 0.0004, iters: 64, batch: 200, momentum: 0.9, tol: 1e-4, n_iter_no_change: 10, folds: 2 },
        "mlpr_opt" | "mlpr_opt_cfnade8" => BlendParams { hidden: vec![32, 32], alpha: 0.24101624563575427, lr: 0.0005339808669500829, iters: 80, batch: 200, momentum: 0.9148116502629432, tol: 2.890613343488106e-5, n_iter_no_change: 9, folds: 2 },
...
fn main() -> ExitCode {
    ...
    println!(
        "MLP:       hidden={:?} alpha={} lr={} iters={} batch={} momentum={} tol={} patience={}",
