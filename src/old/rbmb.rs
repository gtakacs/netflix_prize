// Conditional RBM for collaborative filtering — the production variant: mini-batched
// over users with momentum + weight decay, a CD-k ramp, a conditional D matrix over
// the rated/unrated vector, and per-user visible bias bu[u, k] plus per-user-day
// bias but[u, day, k]. Optional rayon path and a p(h|V) prediction cache.
// Produced: rbmb-256 (+ _ep01/_ep11), rbmb-512x, rbmb-2048x, rbmb-4096x.
// Frozen archive — see README.md; superseded by src/rx.rs, does not build.
...
impl RbmModel {
    ...
    fn get_hidden_probs(&self, u: usize) -> Vec<f32> {
        let ratings = &self.user_ratings[u];
        let r_items = self.user_r_items.as_ref().map(|v| v[u].as_slice());
        if let Some(cache) = &self.pred_cache {
            let mut guard = cache.lock();
            if let Some(ph) = &guard[u] {
                return ph.clone();
            }
    ...
    fn predict_probs(&self, u: usize, i: usize, day: i16) -> Array1<f32> {
        let p_h = self.get_hidden_probs(u);
        let eff_bu = self.effective_bu_day(u, day);
        probs_with_ph(&self.bv, &self.w, i, &p_h, self.cfg.n_hidden, &eff_bu)
    }
}
...
fn sample_hidden(rng: &mut StdRng, probs: &[f32]) -> Vec<f32> {
...
fn accumulate_user_grad(
    ...
    // bu/but do NOT affect p(h|v)
    let p_h_pos = hidden_probs_from_state(bh, w, d, ratings, &v_state, r_items, r_contrib, n_hidden);

    let mut p_h = p_h_pos.clone();
    for _ in 0..cd_steps {
        let h_sample = sample_hidden(rng, &p_h);
        ...
        // bu/but do NOT affect p(h|v)
        p_h = hidden_probs_from_state(bh, w, d, ratings, &v_state, r_items, r_contrib, n_hidden);
    }
    let p_h_neg = p_h;
    ...
    for (idx, (item, rating, day_idx)) in ratings.iter().enumerate() {
        ...
        for j in 0..n_hidden {
            grad_w[[*item, k_data, j]] += p_h_pos[j];
            grad_w[[*item, k_model, j]] -= p_h_neg[j];
        }
    ...
    for j in 0..n_hidden {
        grad_bh[j] += p_h_pos[j] - p_h_neg[j];
    }

    if let (Some(r_items), Some(gd)) = (r_items, grad_d.as_mut()) {
        for &item in r_items {
            for j in 0..n_hidden {
                gd[[item, j]] += p_h_pos[j] - p_h_neg[j];
            }
        }
    }
...
impl Regressor for RbmModel {
    ...
    fn new(tr: &Dataset, pr: &Dataset, cfg: Self::Config) -> Self {
        ...
        let w = rand_array3(tr.n_items, K, cfg.n_hidden, &mut rng, 0.01);
        let bv = init_visible_biases(tr);
        let bh = Array1::<f32>::zeros(cfg.n_hidden);

        let d = if cfg.use_conditional {
            Some(rand_array2(tr.n_items, cfg.n_hidden, &mut rng, 0.01))
        } else {
            None
        };
    ...
    fn predict(&self, u: usize, i: usize, day: i32) -> f32 {
        ...
        predict_with_ph(&self.bv, &self.w, i, &p_h, self.cfg.n_hidden, &eff_bu)
    }
...
fn main() {
    ...
    gravity::fit2::<RbmModel>(
        cfg,
        "rtg",        // target
        "rbmub-512",  // model_name
        false,        // save_subscores
        true,         // save_train
        true,         // save_probe_each_epoch
    );
}
