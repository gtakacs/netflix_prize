// tsvd.rs + per-day NSVD1: a second implicit-feedback term s_{u,day} built only
// from the items the user rated on that same day, so the prediction becomes
// mu + b_u(t) + b_i(t) + (p_u + s_u + s_{u,day}) . q_i.
// Produced: tsvdx-20, tsvdx-256, knn-25w__tsvdx-16.  Frozen archive, see README.md.
